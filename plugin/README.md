...
The default installed surface is intentionally restricted to read-only and
preview tools. It is meant for live dialog resolution, reading, searching,
context collection, and non-sending reply drafts.
...
- exposes only the default facade allowlist for read/search/context/draft work
...
Write/export workflows still exist in the broader local Telegram stack, but
they require explicit non-default tooling and their own safety checks. Media
download and voice transcription are scoped local inspection tools in the
default allowlist.
