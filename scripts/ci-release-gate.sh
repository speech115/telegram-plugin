...
export TELEGRAM_PLUGIN_SOURCE="${ROOT}/control-plane/generated/telegram-plugin-package"
export TELEGRAM_PLUGIN_PACKAGE="${ROOT}/control-plane/generated/telegram-plugin-package"
...
run_gate packaging-hygiene "${ROOT}/scripts/packaging-hygiene.sh" \
  "${ROOT}/plugin" \
  "${ROOT}/control-plane/generated/telegram-plugin-package" \
...
run_gate mcp-release-gates bash -lc \
  "cd '${ROOT}/mcp' && PYTHONPATH='${ROOT}/mcp/src' '${PYTHON_BIN}' -m telegram_mcp.release_gates --package-dir '${ROOT}/control-plane/generated/telegram-plugin-package' --json"

run_gate plugin-package-build bash -lc "
  tmp=\$(mktemp -d)
  '${ROOT}/mcp/bin/build-plugin-package' \
    --source-dir '${ROOT}/control-plane/generated/telegram-plugin-package' \
    --output-dir \"\${tmp}/out\" \
    --json >/dev/null
  test -f \"\${tmp}/out/.codex-plugin/plugin.json\"
"

CP_ENV="TELEGRAM_CI_PORTABLE=1 TELEGRAM_MONOREPO_ROOT='${ROOT}' TELEGRAM_CONTROL_PLANE_ROOT='${ROOT}/control-plane' TELEGRAM_MCP_REPO='${ROOT}/mcp' TELEGRAM_PLUGIN_SOURCE='${ROOT}/control-plane/generated/telegram-plugin-package' TELEGRAM_PLUGIN_PACKAGE='${ROOT}/control-plane/generated/telegram-plugin-package' TELEGRAM_PROJECTS_ROOT='${ROOT}' PYTHONPATH='${ROOT}/control-plane/src:${ROOT}/mcp/src'"
