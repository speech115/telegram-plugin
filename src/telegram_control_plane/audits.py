...
def _top_slow_tools(tool_latency: dict[str, Any], *, limit: int = 10) -> list[dict[str, Any]]:
...
def audit_mcp_telemetry(*, window_hours: float | None = None) -> dict[str, Any]:
    ...
    for bucket in tool_error_buckets:
        ...
        if str(bucket.get("tool")) not in {
            ...
            continue
        error_type = str(bucket.get("error_type") or "")
        if error_type not in {"FloodWaitError", "PeerFloodError"}:
            continue
        count = bucket.get("count")
        read_floodwait_events += int(count) if isinstance(count, int | float) else 1
    if (
        isinstance(max_read_floodwait_events, int | float)
        and read_floodwait_events > max_read_floodwait_events
    ):
        findings.append(
            {
    ...
    if isinstance(max_lane_rate_limited, int | float):
        for lane, lane_stats in _telemetry_stats_lanes().items():
    ...
    return {
        ...
        "top_slow_tools": _top_slow_tools(tool_latency),
